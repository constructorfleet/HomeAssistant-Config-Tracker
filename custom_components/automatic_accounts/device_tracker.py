...
class AutomaticAccount:
    ...
    def __init__(self, name, hass, filename, client, session, async_see, include_devices, configurator_request_id = None):
        self.hass = hass
        self.name = name
        self.client = client
    ...
    def initialize_callback(self, code, state):
        ...
        try:
            ...
            _LOGGER.error(str(err))
            self.configurator.async_notify_errors(request_id, str(err))
            return False
